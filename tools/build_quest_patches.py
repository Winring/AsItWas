...
Assignment:
  Default is first QuestV2 snapshot (when the ID exists in the client).
  If QuestPOIBlob lists that QuestID later, use the POI patch only when it is
  still the same expansion major (grey / spoiler-bridge, e.g. 92924: 12.0.1 ID,
  12.1.0 POI). A POI that first appears in a later expansion is a dump backfill
  (10.1.5 filled QuestID on old blobs) — keep QuestV2. Salvaging a Disaster
  (49232) is BfA 8.0.1, not DF 10.1.5.
  If the quest never got a map POI, fall back to QuestV2 (source=id_only).
...
PROBES = {
    ...
    49232: "8.0.1",  # Salvaging a Disaster: ID in 8.0.1, POI blob backfilled 10.1.5
}
...
def same_expansion(a: str, b: str) -> bool:
    return patch_tuple(a)[0] == patch_tuple(b)[0]


def assign_patch(first_id: str, first_poi: str | None) -> str:
    """QuestV2 first-seen, unless a later POI is still the same expansion."""
    if first_poi is None:
        return first_id
    if patch_tuple(first_poi) > patch_tuple(first_id) and same_expansion(
        first_id, first_poi
    ):
        return first_poi
    return first_id
...
def source_label(
    ...
    if poi_first and patch_tuple(poi_first) > patch_tuple(id_first) and assigned != poi_first:
        return "poi_backfill"
...
def write_lua(
    ...
    lines = [
        ...
        "-- Value is first QuestV2; later QuestPOIBlob only if same expansion major.",
        f"-- Snapshots: {snap}",
...
def main() -> int:
    ...
    parser.add_argument(
        "--from-csv",
        action="store_true",
        help="Re-assign from existing quest_patches.csv (no wago download).",
    )
    ...
    if args.from_csv:
        if not csv_path.exists() or not meta_path.exists():
            print("Need existing quest_patches.csv and quest_patches_meta.json.", file=sys.stderr)
            return 1
        meta = json.loads(meta_path.read_text(encoding="utf-8"))
        newest_build = meta["newest_build"]
        baseline = meta["baseline"]
        snapshots = [(item["patch"], item["build"]) for item in meta["snapshots"]]
        rows: list[tuple[int, str, str, str | None, str]] = []
        ...
        with csv_path.open(newline="", encoding="utf-8") as handle:
            for row in csv.DictReader(handle):
                ...
                first_id = row["id_first"]
                raw_poi = (row.get("poi_first") or "").strip()
                first_poi = raw_poi or None
                patch = assign_patch(first_id, first_poi)
                src = source_label(patch, first_id, first_poi, baseline)
                counts[src] = counts.get(src, 0) + 1
                assigned[qid] = patch
                rows.append((qid, patch, first_id, first_poi, src))
        write_csv(csv_path, rows)
        write_lua(lua_path, rows, newest_build, snapshots)
        meta["quest_count"] = len(rows)
        meta["counts"] = counts
        meta_path.write_text(json.dumps(meta, indent=2) + "\n", encoding="utf-8")
        print(f"Reassigned {csv_path} ({len(rows)} quests)")
